...
class EnhancedRetrievalTrainer:
    ...
    def train_step(self, batch):
        ...
        # Unpack batch -- NEW (paper2): batch is a dict carrying the extra section
        # fields (findings_token_count/has_find/has_imp) from data_loader_v1_paper2.py,
        # alongside the existing 'images'/'captions'. Falls back to a plain
        # (images, texts) tuple (no section info) for backward compatibility.
        ...
        # Forward pass - get all embeddings (+ section-aware pooled outputs when available)
        has_section_info = findings_token_count is not None
        if has_section_info:
            (image_emb, text_emb, synergy_img_emb, synergy_txt_emb, diff_img_emb, diff_txt_emb,
             z_find, z_imp) = self.model(
                (images, texts),
                training=True,
                ...
                has_find=has_find,
                has_imp=has_imp,
                token_ids=texts,  # token_ids is just the existing caption tensor -- not duplicated
            )
        else:
        ...
        # Original 3 terms and weights kept EXACTLY as before.
        original_total_loss = 0.65 * synergy_loss + 0.2 * main_loss + 0.15 * orthogonal_loss

        # NEW (paper2): granularity loss, using the already-verified compute_granularity_loss.
        # Only computed when section info was actually available for this batch.
        if has_section_info:
            gran_loss, find_loss, imp_loss, n_find, n_imp = compute_granularity_loss(
                image_emb, z_find, z_imp, has_find, has_imp
            )
        else:
            gran_loss = torch.zeros((), device=image_emb.device)

        total_loss = original_total_loss + 0.10 * gran_loss
        ...
        return (total_loss.item(), synergy_loss.item(), difference_loss.item(), orthogonal_loss.item(),
                main_loss.item(), gran_loss.item())
    ...
    def evaluate_recall_k_batched(self, data_loader, batch_size=100, k=[1, 5, 10], max_samples=None):
        ...
        for i in range(similarity_matrix.size(0)):
            # Find rank of correct match for image-to-text
            sim_scores = similarity_matrix[i]
            correct_rank = torch.where(torch.argsort(sim_scores, descending=True) == i)[0][0] + 1
            ranks.append(correct_rank.item())
        
        for i in range(similarity_matrix.size(1)):
            # Find rank of correct match for text-to-image
            sim_scores = similarity_matrix[:, i]
            correct_rank = torch.where(torch.argsort(sim_scores, descending=True) == i)[0][0] + 1
            ranks.append(correct_rank.item())
    ...
    def get_resume_checkpoint_path(self):
        """
        NEW (paper2): fixed path for the resumable checkpoint (model +
    ...
    def save_checkpoint(self, epoch):
        """
        NEW (paper2): save model weights + optimizer state_dict + current
    ...
    def train(self, train_loader, val_loader, num_epochs):
        ...
        # NEW (paper2): resume from an existing checkpoint if one exists for this experiment.
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for step, batch in pbar:
                ...
                # NEW (paper2): also move the section fields, when present.
                if isinstance(batch, dict):
                    step_batch = {
                        'images': batch['images'].to(self.device),
                        'captions': batch['captions'].to(self.device),
                    }
                    if 'findings_token_count' in batch:
                        step_batch['findings_token_count'] = batch['findings_token_count'].to(self.device)
                        step_batch['has_find'] = batch['has_find'].to(self.device)
                        step_batch['has_imp'] = batch['has_imp'].to(self.device)
                else:
                    images, texts = batch
                    step_batch = (images.to(self.device), texts.to(self.device))

                # Forward pass
                total_loss, synergy_loss, difference_loss, orthogonal_loss, main_loss, gran_loss = self.train_step(
                    step_batch
                )
                ...
                pbar.set_description(
                    f"Total: {total_loss:.4f}, Syn: {synergy_loss:.4f}, Main: {main_loss:.4f}, "
                    f"Ortho: {orthogonal_loss:.4f}, Gran: {gran_loss:.4f}"
                )
            ...
            print(f"   Granularity Loss: {epoch_gran_loss:.6f}")  # NEW (paper2)
            ...
            # NEW (paper2): checkpoint every 3rd epoch, or the final epoch (so the
            # last epoch's progress is never lost even if num_epochs isn't a
            # multiple of 3). Same fixed filename each time -- overwrite, not append.
            ...
            self.history['gran_loss'].append(epoch_gran_loss)  # NEW (paper2)
