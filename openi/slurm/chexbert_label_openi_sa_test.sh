#!/bin/bash
#SBATCH --job-name=chexbert_openi_sa
#SBATCH --output=logs/chexbert_openi_sa_%j.out
#SBATCH --error=logs/chexbert_openi_sa_%j.err
...
IN=$PROJ/openi/data/chexbert_input_test.csv
OUT=$PROJ/openi/data/chexbert_out
...
echo "===== FINAL SUMMARY ====="
echo "input_rows=$(($(wc -l < "$IN")-1)) output_rows=$(python3 -c "import pandas as pd; print(len(pd.read_csv('$OUT/labeled_reports.csv')))")"
