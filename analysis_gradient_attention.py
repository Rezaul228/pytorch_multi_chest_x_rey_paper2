...
def find_reports_with_medical_term(data_loader, tokenizer, target_term, num_samples=12):
    """Find reports containing a specific medical term"""
    ...
    for batch in val_loader:
        ...
        # Check for target term
        if (target_term.lower() in text_description and
...
def create_single_term_visualization(model, found_reports, tokenizer, target_term, save_dir='gradient_attention_analysis'):
    ...
    for i in range(num_reports):
        ...
        if img_to_txt_attn is not None and txt_to_img_attn is not None:
            ...
            for j, word in enumerate(words):
                if target_term.lower() in word.lower():
                    target_word_idx = j
                    break
...
def create_minimal_attention_visualization(model, found_reports, tokenizer, save_dir='gradient_attention_analysis'):
...
def create_minimal_attention_visualization(model, found_reports, tokenizer, target_term, save_dir='gradient_attention_analysis'):
