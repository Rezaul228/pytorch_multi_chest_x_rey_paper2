#!/bin/bash
#SBATCH --job-name=a2_2553_MGG2L_s123
#SBATCH --output=logs/a2_scale2553_MGG2L_seed123_%j.out
#SBATCH --error=logs/a2_scale2553_MGG2L_seed123_%j.err
...
#
#   size 2553: 20 steps/epoch x 1522 epochs = 30440 gradient steps
...
mkdir -p logs
echo "=============================================================="
echo "BUDGET: size=2553  steps_per_epoch=20  epochs=1522  total_planned_steps=30440"
echo "        full-MIMIC reference = 30450 steps  (deviation $(python3 -c "print(f'{100*(30440-30450)/30450:+.2f}%')"))"
...
python3 -u train_retrieval_v2_paper2.py \
    --experiment_name mimic_shards_hybrid_full_ori_vo10805_to128_lr1e-4_b128_ep1522_dualbr_sy065_main_loss20_ortho15__branch_MGG2L_paper2_scale2553_seed_123 \
    --dataset_mode mimic_shards_hybrid_full_ori \
    --train_ids mimic/data/scale_ablation/train_ids_2553.csv \
