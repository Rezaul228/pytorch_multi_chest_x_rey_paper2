...
class TrainingVisualizer:
    ...
    def plot_dual_branch_losses(self):
        ...
        # Plot 1: Synergy Loss Only
        plt.subplot(1, 2, 1)
        
        # Add synergy loss if available
        if 'synergy_loss' in self.history and len(self.history['synergy_loss']) > 0:
            plt.plot(epochs, self.history['synergy_loss'], 'g-', linewidth=2, marker='o', markersize=4, label='Synergy Loss')
        
        plt.xlabel('Epoch', fontsize=12)
        plt.ylabel('Loss', fontsize=12)
        plt.title('Synergy Loss', fontsize=12, fontweight='bold')
        plt.grid(True, alpha=0.3)
        plt.legend()
        
        # Plot 2: Difference Loss Only
        plt.subplot(1, 2, 2)
        
        # Add difference loss if available
        if 'difference_loss' in self.history and len(self.history['difference_loss']) > 0:
            plt.plot(epochs, self.history['difference_loss'], 'r-', linewidth=2, marker='s', markersize=4, label='Difference Loss')
        
        plt.xlabel('Epoch', fontsize=12)
        plt.ylabel('Loss', fontsize=12)
        plt.title('Difference Loss', fontsize=12, fontweight='bold')
        plt.grid(True, alpha=0.3)
        plt.legend()
...
def create_ablation_performance_comparison(results, save_dir, timestamp):
    ...
    # Increase figure size and adjust layout for better label visibility
