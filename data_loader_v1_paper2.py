...
def load_section_boundaries(split_name):
    """
    NEW (paper2): read section_boundaries_{split_name}_paper2.csv ONCE into
    an in-memory dict: study_id (str) -> (findings_token_count, has_find, has_imp).

    Returns (section_boundaries_dict, csv_path). If the file doesn't exist
    (or split_name is unknown), returns an empty dict -- every sample will
    ...
    if csv_path is None or not os.path.exists(csv_path):
        print(f"Warning: section boundary file not found ({csv_path}). "
              f"All samples in this dataset will use the fallback (has_find=False, has_imp=False).")
        return section_boundaries, csv_path

    print(f"Loading section boundaries ONCE from: {csv_path}")
    df = pd.read_csv(csv_path, dtype={'study_id': str})
    for row in df.itertuples(index=False):
        section_boundaries[row.study_id] = (
            int(row.findings_token_count),
            bool(row.has_findings),
            bool(row.has_impression),
        )
    print(f"Loaded {len(section_boundaries)} section-boundary entries for split='{split_name}'")
    return section_boundaries, csv_path
...
class IndianaDataset(Dataset):
    def __init__(self, shard_paths, max_samples=None, split_name=None):
        ...
        # (i.e. once per training run for this split), not per batch/sample.
        self.split_name = split_name or self._infer_split_name(shard_paths)
        self.section_boundaries, self.section_boundaries_path = load_section_boundaries(self.split_name)
        self.fallback_count = 0  # count of study_ids not found in section_boundaries

        self._load_samples()

    @staticmethod
    def _infer_split_name(shard_paths):
        """Infer 'train'/'val'/'test' from the shard files' parent directory name."""
        if not shard_paths:
            return None
        return os.path.basename(os.path.dirname(shard_paths[0]))

    def reset_fallback_count(self):
        """Call at epoch boundaries if you want a per-epoch (rather than cumulative) count."""
    ...
    def __getitem__(self, idx):
        ...
        study_id_str = str(sample['study_ids'])

        # NEW (paper2): attach findings_token_count/has_find/has_imp for this
        # sample. token_ids for downstream use is just the existing 'captions'
        # tensor below -- not duplicated here.
        if study_id_str in self.section_boundaries:
            findings_token_count, has_find, has_imp = self.section_boundaries[study_id_str]
        else:
            # SAFETY FALLBACK: never crash on a missing study_id -- fall back to
            # Paper 1's original behavior for just this one sample.
            findings_token_count, has_find, has_imp = 0, False, False
        ...
        return {
            ...
            'findings_token_count': torch.tensor(findings_token_count, dtype=torch.long),
            'has_find': torch.tensor(has_find, dtype=torch.bool),
            'has_imp': torch.tensor(has_imp, dtype=torch.bool),
        }
...
class IndianaDataLoader:
    ...
    def get_test_data(self, num_samples=None):
        ...
        test_dataset = IndianaDataset(test_shards, num_samples, split_name='test')
