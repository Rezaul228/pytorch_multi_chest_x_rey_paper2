...
from base_models_refactored_v1 import MultimodalFusion, ContrastiveLoss
...
class EnhancedRetrievalTrainer:
    ...
    def __init__(self, model, temperature=None, learning_rate=1e-4, 
        ...
        # Initialize optimizer with fixed learning rate and weight decay
        ...
        # No scheduler - using fixed learning rate
        self.scheduler = None
    ...
    def train_step(self, batch):
        """Single training step with proper dual branch loss calculation"""
        self.model.train()
        ...
        # Compute individual branch losses for monitoring
        synergy_loss = self.loss_fn(None, (synergy_img_emb, synergy_txt_emb))
        difference_loss = self.loss_fn(None, (diff_img_emb, diff_txt_emb))
        
        # Main loss using final combined embeddings (already normalized in model)
        total_loss = self.loss_fn(None, (image_emb, text_emb))
    ...
    def save_model(self):
        ...
        try:
            ...
            # Also save weights in original location for backward compatibility
